 ...
 * Arithmetic
 *
 * For the next section, we're going to take a different tack
 * with the solutions. We'll declare a new class, S99Int, and
...
class S99Int(val start: Int) {
   ...
   */
  def isPrime : Boolean =
    (start > 1) && (primes.takeWhile(_ <= Math.sqrt(start).toInt).forall(start % _ != 0))
  ...
  def primeFactors : List[Int] =
    if (start < 1) {
      throw new ArithmeticException(s"$start is not a positive integer")
    } else {
      def helper(n : Int, ps : Stream[Int]) : List[Int] = {
        if (n == 1)
          List()
        else if (n % ps.head == 0)
          (ps.head :: helper(n / ps.head, ps))
        else
          helper(n, ps.tail)
      }
      helper(start, primes)
  ...
  def primeFactorMultiplicity : List[(Int, Int)] = {
    if (start < 1) {
      ...
      def helper(n : Int, ps : Stream[Int], factors : List[(Int, Int)]) : List[(Int, Int)] = {
        if (n == 1) {
          factors
        } else if (n % ps.head != 0) {
          helper(n, ps.tail, factors)
        } else {
          val newFactors = factors match {
            case (factor, count) :: rest if factor == ps.head => (factor, count+1) :: rest
            case _ => (ps.head, 1) :: factors
          }
          helper(n / ps.head, ps, newFactors)
        }
      }
      helper(start, primes, List()).reverse
    }
  }
  ...
  def totient2 : Int = {
    ...
    primeFactorMultiplicity.map{ _ match {
      case (prime, multiplicity) => (prime - 1) * Math.pow(prime, multiplicity - 1).toInt
    }}.foldLeft(1)(_ * _)
  }
   ...
   * facts in number theory that has not been proved to be correct in the
  ...
  def goldbach : (Int, Int) = {
    primes.takeWhile(_ < start).find(p => (start - p).isPrime) match {
      ...
      case None => throw new ArithmeticException("No goldbach pair found for ${start}")
    }
  }
...
object S99Int {
   ...
   */
  def listPrimesInRange(r : Range) = r.filter(_.isPrime).toList
   ...
   * 1928 = 61 + 1867
   */
  def printGoldbachList(ns : Traversable[Int]) {
    printGoldbachListLimited(ns, 0)
  }

  def printGoldbachListLimited(ns : Traversable[Int], limit : Int) {
    ns.filter { n => (n > 2 && n % 2 == 0) }
      .map { _.goldbach }
      .filter { p => (p._1 >= limit && p._2 >= limit) }
      .foreach { p => println(s"${p._1 + p._2} = ${p._1} + ${p._2}") }
  }
}